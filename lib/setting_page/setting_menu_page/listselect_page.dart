...
class _ListselectPageState extends State<ListselectPage> {
  ...
  Future<void> _pickCsvFile(String type) async {
    ...
    try {
      ...
      setState(() {
        _filePaths[type] = path;
      });
      // 設定完了ダイアログを表示
      fileSettingOkDialog();
    } catch (e) {
      // たとえばファイル読み込み中に例外が発生したらエラー表示
      _showErrorDialog('CSV の読み込み中にエラーが発生しました: $e');
    }
  }
  ...
  Future<void> _showErrorDialog(String message) async {
    ...
    Widget _buildSection(String type) {
      final fileName = _filePaths[type]
          ?.split('/')
          .last;
      return Column(
                    ...
                    children: [
                      ...
                      if (_allFilesSelected) ...[
                        ...
                        ElevatedButton.icon(
                          ...
                          label: Text('すべてクリア', style: TextStyle(
                              fontWeight: FontWeight.bold)),
                          style: ElevatedButton.styleFrom(
                            backgroundColor: Colors.redAccent,
